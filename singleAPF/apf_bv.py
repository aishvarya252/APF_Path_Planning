...
class APF:
    ...
    def __init__(self, k_att, k_rep, r_apf, k_replane, k_lane, k_car, lane_width, target_area, F_e, del_t, max_iters, solid_lines, transition_lines, obstacles, vehicles, L, B, car, num_vehicles):
        # Force parameters
...
If the vehicle enters a "local minimum," emergency forces are applied using the EMERGENCY class.'''
        ...
        F_obs = [F_obsx, F_obsy]
        print("obs Repulsion force for vehicle: ",F_obs)
...
Uses exponential decay functions for smooth force gradients.'''
    ...
    def pathplanning(self, rear_p_list, front_p_list, fi_list, targets, iters_list, moving_vehicles):
        ...
        while not all(vehicles_reached_target): 
            ...
            for i in range(len(rear_p_list)):
                ...
                print(f"IN PATHPLANNING: VEHICLE {i} - Rear: {rear_p}, Front: {front_p}, Orientation: {fi}, Target: {target}")
            ...
            # Calculate initial distance to the target
                ...
                if distance <= self.target_area:
                    ...
                    print(f"\n----------\nVehicle {i + 1} has reached the target!\n----------")
                    forces.append(None)
                    steering_angles.append(None)
                    continue
                print(f"\n----------\nVehicle {i + 1}: Iteration {iters_list[i]}")
            ...
            # **Step 3: Draw Both Vehicles in the Same Frame**
            for i in range(len(rear_p_list)):
                if vehicles_reached_target[i]:  # Skip already reached vehicles
                    continue
        ...
        print("\nAll vehicles have reached their targets! Simulation stopping.")
