...
class MAP:
    ...
    def draw_boundaries(self, fig, boundaries):
        for boundary in boundaries:
            x_vals, y_vals = zip(*boundary)  # Extract x & y coordinates
            plt.plot(x_vals, y_vals, c='black', linewidth=2)

    def draw_obstacles(self, fig, obstacles):
        for obstacle in obstacles:
            x_vals, y_vals = zip(*obstacle)
            plt.fill(x_vals, y_vals, color='red', alpha=0.5)
