...
class APF:
    ...
    def __init__(self, k_att, k_rep, r_apf, k_replane, k_lane, k_car, lane_width, target_area, F_e, del_t, max_iters,
        ...
        self.k_att = k_att           #Determines the strength of attraction towards the target.
        self.k_rep = k_rep
        self.r_apf = r_apf           #Defines the radius of influence for obstacles.
        self.k_replane = k_replane
        self.k_lane = k_lane         #Strength of forces to keep the vehicle in the lane.
        self.k_car = k_car           #Strength of repulsion from other vehicles.
        self.lw = lane_width
        self.target_area = target_area   #The acceptable distance from the target to consider it reached.
        self.F_e = F_e
        self.del_t = del_t
        ...
        self.F_vehs = []
        self.F_totals = []
        '''self.rear_velocities = [[] for _ in range(2)]  # Store for 2 vehicles
        self.paths = [[] for _ in range(2)]
        self.directions = [[] for _ in range(2)]
        self.front_velocities = [[] for _ in range(2)]
        self.front_points = [[] for _ in range(2)]
        self.steering = [[] for _ in range(2)]
        self.F_totals = [[] for _ in range(2)]'''
...
Uses exponential decay functions for smooth force gradients.'''
    ...
    def pathplanning(self, rear_p, front_p, fi, target, iters, moving_vehicles):
        #Creates SUPPLY and EMERGENCY objects for lane and obstacle handling.
        ...
        while distance > self.target_area and iters <= self.max_iters:
            ...
            #Determine the direction of the total force (F_direction).
            #lculate the steering angle (si) to align the vehicle's heading (fi) with F_direction.
