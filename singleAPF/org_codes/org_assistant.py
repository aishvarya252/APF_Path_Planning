...
class MAP:
    ...
    def final_draw(self, fig, front_points, path, init_movingvehis, moving_vehicles, L, B):
        ...
        for i in front_points:
            #front_points: Draws green dots for the vehicle's front points during the simulation.
            front_x.append(i[0])
            front_y.append(i[1])
            plt.plot(front_x, front_y, 'o', c='green', markersize=2)
        for j in path:
            #Draws the vehicle's trajectory as a red line.
            path_x.append(j[0])
            path_y.append(j[1])
        # plt.plot(front_x, front_y, c='green', linewidth=2)
        #Shows moving vehicles using green rectangles with transparency (alpha=0.3)
        ...
        for vehicle in moving_vehicles:
            #visualizes the current positions of the moving vehicles:
            if vehicle[2] == 'h':
                x1 = vehicle[0] - L / 2
                y1 = vehicle[1] - B / 2
